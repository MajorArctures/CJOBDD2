-----------------------------------------------------------------------------------
-- Prova P2 - 2024
-- Prova P2 - Beatriz.sql
-- Solução da Prova P2
-----------------------------------------------------------------------------------
-----------------------------------------------------------------------------------
-- Disciplina: Banco de Dados 2 (CJOBDD2)
-- Nome do aluno: Beatriz Helena e Silva Pinto
...
SELECT 
	MAX(ValorUnitario)	AS	'Valor Unitário MÁXIMO',
	MIN(ValorUnitario)	AS	'Valor Unitário MÍNIMO'
FROM PRODUTOS;
GO
...
SELECT
	...
	Quantidade		AS	'Quantidade Solicitada'
FROM ITEM_DE_PEDIDO
WHERE Quantidade = '35';
GO
...
SELECT 
	...
	Salario				AS	'Salario do Vendedor', 
...
SELECT 
    ...
    COUNT(DISTINCT i.CodigoProduto) AS 'Quantidade de Produtos Diferentes',
    SUM(i.Quantidade)				AS 'Total de Itens'
FROM Pedidos p JOIN ITEM_DE_PEDIDO i ON p.NumeroPedido = i.NumeroPedido
...
SELECT 
    c.CodigoCliente						AS 'Código do Cliente', --Tabela cliente
    c.NomeCliente						AS 'Nome do Cliente', --Tabela cliente
    p.DescricaoProduto					AS 'Nome do Produto', --Tabela produto
    p.ValorUnitario						AS 'Valor Unitário', --Tabela produto
    i.Quantidade						AS 'Quantidade', --Tabela item pedido
    (p.ValorUnitario * i.Quantidade)	AS 'Valor Total a Ser Pago',
    pe.PrazoEntrega						AS 'Prazo de Entrega', --Tabela pedido
    v.NomeVendedor						AS 'Nome do Vendedor' --Tabela vendedores
--selecionar de onde tudo vem
FROM 
    Clientes c
    JOIN Pedidos pe ON c.CodigoCliente = pe.CodigoCliente
    JOIN ITEM_DE_PEDIDO i ON pe.NumeroPedido = i.NumeroPedido
    JOIN Produtos p ON i.CodigoProduto = p.CodigoProduto
    JOIN Vendedores v ON pe.CodigoVendedor = v.CodigoVendedor
--ordenar a saída
ORDER BY 
    c.NomeCliente,
    p.DescricaoProduto,
